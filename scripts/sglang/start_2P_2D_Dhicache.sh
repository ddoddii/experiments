#!/bin/bash
# SGLang 2P2D — No hicache (baseline)
# =====================================
# P 노드: hicache 없음 (GPU-only KV, standard prefill)
# D 노드: hicache 없음 (GPU-only KV, standard decode)
#
# 참고: SGLang decode 모드는 --disaggregation-mode decode 사용 시
#   내부적으로 chunk cache(disable-radix-cache)를 강제 설정함.
#   --enable-hierarchical-cache는 radix cache와 충돌 → decode 노드에 hicache 불가.
#   D-only hicache 및 P+D full hicache는 SGLang 0.5.9에서 지원되지 않음.
#
# 용도: hicache 없는 순수 baseline 측정 (P-only hicache start_2P_2D.sh와 비교)
#
# 벤치마크:
#   CONFIG=sglang_2p2d_nohicache python benchmark/sglang_BFCL_v3_multi_turn_base.py
#   CONFIG=sglang_2p2d_nohicache_c4 CONCURRENCY=4 \
...
echo "[4/6] Starting Decode server 1 (GPU 2, port 30002) — NO hicache..."
...
echo "[5/6] Starting Decode server 2 (GPU 3, port 30003) — NO hicache..."
...
# No hicache — HICACHE_DIRS empty, exporter tracks API metrics only
HICACHE_DIRS="" \
...
echo ""
echo "All done. Config: No hicache (baseline)"
echo "  P1/P2: no hicache (GPU-only KV)"
echo "  D1/D2: no hicache (GPU-only KV)"
echo "  Note: SGLang decode mode forces chunk cache — D-node hicache is not supported"
echo ""
echo "Benchmark:"
echo "  CONFIG=sglang_2p2d_nohicache python benchmark/sglang_BFCL_v3_multi_turn_base.py"
echo "  CONFIG=sglang_2p2d_nohicache_c4 CONCURRENCY=4 python benchmark/sglang_BFCL_v3_multi_turn_concurrent.py"
echo ""
