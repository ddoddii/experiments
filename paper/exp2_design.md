...
### M0. 지표 자체의 함정 (1차 프로브가 여기서 걸렸다)

`sglang:token_usage`는 **prefix cache를 사용량에서 뺀다** (`_get_token_info`:
`num_used = max_total − (available + evictable)`). 축출 가능한 블록은 admission control
관점에선 "여유"이므로 그 정의가 틀린 건 아니지만, **캐시로 가득 찬 prefill 풀이 점유율
~0으로 보고된다.**

1차 프로브(`results/exp2/pd_layoutb_p60000_c16/`)가 정확히 이걸로 M1=0을 냈다. P0/P1이
`use_mean=0.019`였는데, 같은 60k 풀에서 Exp 1의 hit rate가 55.7%였으므로 **모순**이다.
위험한 건 실패 방향이다 — 캐시를 무시하는 지표는 *불균형 없음*을 보고하고, 그건 정당한
"전제가 틀렸으니 접어라"와 구분되지 않는다.

→ fork에 `sglang:cache_occupancy = (used + evictable) / capacity` 추가.
샘플러가 이걸 주지표로 쓰고 `token_usage`는 `*_pressure` 컬럼에 함께 기록한다.
**둘의 차이가 곧 prefix cache**이므로 그 자체가 보고 가치가 있다.

### M1. 낭비된 헤드룸 (전제 확립, `hicache` arm에서 측정)

> **주의 — M1은 부분적으로 자명해질 수 있다.** radix cache는 빈 공간이 있으면 채우므로,
> `cache_occupancy` 기준 P는 거의 항상 ~1.0이 된다. 그러면 M1은 사실상 "D의 여유 × 실행
> 시간"이 되고 "P가 압박받았다"는 조건이 무의미해진다.
> **100% 점유 ≠ 유용한 데이터를 축출 중**이다 (hit rate 100%인 꽉 찬 캐시는 압박이 아니다).
> → 반드시 **축출이 실제로 비용을 물리고 있다는 증거와 짝지어야** 한다:
> Exp 1의 hit rate 55.7% = **프롬프트 토큰의 44%를 매번 재계산**. 이게 진짜 압박 증거다.
> 논문 문장은 "P는 프롬프트의 44%를 재-prefill하는 동안 D에 42.6 GB가 놀고 있었다"가 되어야
> 하고, "P가 100% 찼다"가 되어선 안 된다.
