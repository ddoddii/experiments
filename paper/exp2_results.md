...
> **그림**: `results/exp2/fig_exp2.{pdf,png}` (`benchmark/plot_exp2.py`, v5 데이터 기준)
...
M1/M2 막대가 아니라 **결정 로그** 기반 3패널. §3의 이유로 M1/M2는 이 그림에 쓸 수 없다.

- **(a) 메커니즘** — 결정 시점 후보 GPU 점유율의 분포, 선택 vs 거부. 강하게 이봉형으로
  갈라진다 (선택 0.14, 거부 0.53). 널 모델 3종을 참조선으로 겹쳐 그려
  **"우연이 아님"을 같은 패널에서 보인다** (random/round-robin 0.40, always-local 0.88)
- **(b) 이득** — cache hit rate 47.1% → 55.7%
- **(c) 대가** — TTFT p50과 **p95를 반드시 함께**. p50만 그리면 p95가 부정하는 승리를
  보고하게 된다

패널 (a)가 이 실험의 핵심이다: **거부된 후보**가 있어야 "여유 있는 곳에 놓았다"가
반증 가능한 주장이 된다. 모든 후보가 한가했다면 어디에 놓든 한가한 곳에 놓인다.
